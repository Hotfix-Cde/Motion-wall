...
class VideoPreviewController(
    ...
    fun setSettings(newSettings: MotionSettings) {
        ...
        applySettingsToPlayer(); applyTransform()
    }

    fun setVideo(uri: Uri?) {
        if (sourceUri == uri && (uri == null || player != null)) { applySettingsToPlayer(); applyTransform(); return }
        sourceUri = uri
        if (uri == null) { releasePlayer(); placeholderView.isVisible = true; return }
    ...
    fun resume() {
        ...
        if (currentPlayer != null && !currentPlayer.isPlaying) runCatching { currentPlayer.start() }
        else if (currentPlayer == null && sourceUri != null && textureView.isAvailable) preparePlayer()
    }
    ...
    override fun onSurfaceTextureSizeChanged(surfaceTexture: android.graphics.SurfaceTexture, width: Int, height: Int) {
        viewWidth = width; viewHeight = height; applyTransform()
    }
    ...
    private fun preparePlayer() {
        ...
        player = MediaPlayer().apply {
            ...
            setOnPreparedListener { preparedPlayer ->
                videoWidth = preparedPlayer.videoWidth; videoHeight = preparedPlayer.videoHeight
                applySettingsToPlayer(); applyTransform()
                if (textureView.isShown) runCatching { preparedPlayer.start() }
            }
            setOnVideoSizeChangedListener { _, width, height ->
                videoWidth = width; videoHeight = height; applyTransform()
            }
            setOnInfoListener { _, what, _ ->
                if (what == MediaPlayer.MEDIA_INFO_VIDEO_RENDERING_START) placeholderView.isVisible = false
                false
            }
            setOnErrorListener { _, _, _ -> placeholderView.isVisible = true; true }
            prepareAsync()
        }
    }
    ...
    private fun applySettingsToPlayer() {
        ...
        runCatching {
            ...
            currentPlayer.setVideoScalingMode(if (settings.scaleMode == ScaleMode.FIT) MediaPlayer.VIDEO_SCALING_MODE_SCALE_TO_FIT else MediaPlayer.VIDEO_SCALING_MODE_SCALE_TO_FIT_WITH_CROPPING)
        }
    }
    ...
    private fun applyTransform() {
        ...
        val viewW = viewWidth.toFloat(); val viewH = viewHeight.toFloat()
        val scale = if (settings.scaleMode == ScaleMode.CROP) max(viewW / sourceWidth, viewH / sourceHeight) else min(viewW / sourceWidth, viewH / sourceHeight)
